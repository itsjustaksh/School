			...
			const struct nf_hook_state *state)
{
	...
	if(iph->protocol == IPPROTO_ICMP){
		if (iph->daddr==ip_addr){
			printk(KERN_DEBUG "***Dropping %pI4 (ICMP)", &(iph->daddr));
			return NF_DROP; 
		}
	}
	return NF_ACCEPT;
			...
			const struct nf_hook_state *state)
{
	...
	if(iph->protocol == IPPROTO_TCP){
		if (iph->daddr==ip_addr && ntohs(tcph->dest) == 23){
			printk(KERN_DEBUG "***Dropping %pI4 (TCP)", &(iph->daddr));
			return NF_DROP;
		}
	}
	return NF_ACCEPT;
                       ...
                       const struct nf_hook_state *state)
{
   ...
   if (iph->protocol == IPPROTO_UDP) {
       ...
       if (iph->daddr == ip_addr && ntohs(udph->dest) == port){
            printk(KERN_WARNING "*** Dropping by Aksh Ravi %pI4 (UDP), port %d\n", &(iph->daddr), port);
            return NF_DROP;
        }
   }
   return NF_ACCEPT;
                 ...
                 const struct nf_hook_state *state)
{
   ...
   switch (state->hook){
     case NF_INET_LOCAL_IN:     hook = "LOCAL_IN by Aksh Ravi";     break; 
